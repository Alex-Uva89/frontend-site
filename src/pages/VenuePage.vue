<template>
  <q-page class="q-pa-none">
    ...
    <div v-else>
      <HeroImage :images="heroImages" :title="venue?.name || 'Locale'" :subtitle="venue?.address || ''"
        content-bottom="10vh" :loop-duration="30" align="left" />

      <SectionFrame class="q-my-lg">
        <!-- EMOZIONALE (safe) -->
        <EmotionBlock v-if="venue?.emotion" :emotion="venue.emotion" />
      </SectionFrame>
      ...
      <section class="focus">
        <div class="focus__wrap">
          <div class="focus__info">
            ...
            <p class="focus__meta">
              {{ (venue?.kind || '').toUpperCase() }} · {{ venue?.address }}
            </p>
          </div>

          <div class="focus__ctas">
            <q-btn v-if="hasBooking" unelevated no-caps class="btn btn--primary" :href="venue.bookingUrl"
              target="_blank" rel="noopener">
              {{ $t('actions.book', 'Prenota') }}
            </q-btn>

            <q-btn v-if="hasMenu" outline no-caps class="btn btn--ghost" :href="venue.menuUrl" target="_blank"
              rel="noopener">
      ...
      <section class="solutions">
        ...
        <div class="solutions__rail" v-if="isMobile">
          <div class="solutions__track">
            <q-card v-for="s in solutions" :key="s.id" class="sol" clickable v-ripple :to="s.to" :href="s.href"
              :target="s.target" :disable="s.disabled" :tag="s.to ? 'a' : 'a'">
      ...
      <section class="hours">
        ...
        <div class="hours__cta" v-if="hasBooking">
          <q-btn no-caps unelevated class="btn btn--primary" :href="venue.bookingUrl" target="_blank" rel="noopener">
...
const venueCache = new Map() // `${id}:${loc}` → venue
const siteCache = new Map()  // `${loc}` → siteStrings
...
async function load({ soft = false } = {}) {
  ...
  // crea SEMPRE un controller per questo load
  aborter?.abort?.()
...
// ===== CTA helpers =====
const hasBooking = computed(() => !!(venue.value?.bookingUrl && venue.value.bookingUrl !== '#'))
const hasMenu = computed(() => !!(venue.value?.menuUrl && venue.value.menuUrl !== '#'))
...
const solutionsTitle = computed(() =>
  site.value?.solutions?.title || t('venue.solutionsTitle', 'Come vuoi prenotare?')
)
...
const solutions = computed(() => {
  ...
  const bookingHref = v.bookingUrl && v.bookingUrl !== '#' ? v.bookingUrl : ''
...
// normalizza: '24h' | Array<{o,c}> | []
const schedule = computed(() => {
  const raw = venue.value?.hours
  if (!Array.isArray(raw)) return Array(7).fill([])

  return Array.from({ length: 7 }, (_, i) => {
    const d = raw[i]

    // nuovo formato backend: {twentyfour, slots}
    if (d && typeof d === 'object' && !Array.isArray(d)) {
      if (d.twentyfour) return '24h'
      if (Array.isArray(d.slots)) return d.slots.filter(s => s?.o && s?.c)
      return []
    }

    // fallback: già nel formato vecchio
    if (d === '24h') return '24h'
    if (Array.isArray(d)) return d.filter(s => s?.o && s?.c)
...
function formatRange(seg) {
  if (seg === '24h') return site.value?.venueUi?.open24h ?? 'Open 24 hours'
...
:deep(.q-item:not(.today)) .q-item__label {
  opacity: .92;
}

.hours__list .today {
  background: rgba(186, 176, 150, .18);
}
