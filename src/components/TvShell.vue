...
<template>
  ...
  <div class="tv-shell">
    <div class="tv-brand-mobile" aria-hidden="true">
      <span class="tv-brand-text">Mamma Elvira</span>
    </div>
...
.tv-shell::after {
  ...
  background:
    ...
    linear-gradient(180deg, rgba(255, 255, 255, .24), transparent) top 0 left var(--tv-left) / calc(100% - var(--tv-left) - var(--tv-right)) 8px no-repeat,
    linear-gradient(90deg, rgba(255, 255, 255, .18), transparent) top var(--tv-top) left 0 / 8px calc(100% - var(--tv-top) - var(--tv-bottom)) no-repeat;
}
...
@media (max-width:700px) {
  .tv-brand-mobile {
    ...
    right: 0;
    height: var(--tv-top);
    /* occupa esattamente la fascia rossa superiore */
    z-index: 2147483648;
    pointer-events: none;
  }
  ...
  .tv-mobile-cutout {
    ...
    /* TRASPARENTE: si vede il contenuto sotto */
    z-index: 2;
  }
}
</style>
