  ...
  <aside class="tv-controls seventies" aria-label="TV control panel">
    <div class="panel-inner">
      ...
      <div class="display-block glass" aria-label="Text display">
        <svg class="dotmatrix" :viewBox="`0 0 ${charWidth * chars.length} ${charHeight}`"
          preserveAspectRatio="xMidYMid meet" role="img" :aria-label="displayText">
          <g v-for="(ch, ci) in chars" :key="ci" :transform="`translate(${ci * charWidth},0)`">
            <rect v-for="(dot, di) in dotRects(ch)" :key="di" class="dot" :x="dot.x" :y="dot.y" :width="dotSize"
              :height="dotSize" rx="1.5" ry="1.5" />
          </g>
        </svg>
      </div>
      ...
      <div class="knobs">
        ...
        <LanguageKnob :locales="lang.supported" v-model="currentLocale" :width="140" :height="44" />
      </div>
